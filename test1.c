    ...
    for (i = 0; i < MAX_THINGIES; i++) {
        if (thingies[i].valid) {
            for (j = 0; j < thingies[i].size; j++) {
                if (thingies[i].memory[j] != thingies[i].pattern) return false;
            }
        }
    }
    return true;
...
static void test3(bool test_poison) {
    ...
    for (i = 0; i < 224; i++) {
        ...
        if (i % 2) {
            heego(thingies[i].memory);
            // If test_poison and object is smaller than 2048, see if it is
            // poisoned correctly. We should be able to do this safely, since
            // one object will hold the superblock
